...
static void connected(struct bt_conn *conn, uint8_t conn_err)
{
	...
	bt_addr_le_t freebot_addr;
	//freebot mac
	bt_addr_le_from_str("DA:1E:A7:F7:20:F8","random", &freebot_addr);
	//other board mac
	//bt_addr_le_from_str("F6:5E:C0:22:C7:AC","random", &freebot_addr);

	if (!bt_addr_le_eq(bt_conn_get_dst(conn), &freebot_addr)) {
		printk("A device that is not the freebot tried to connect: %s", addr);
